//Author: Mohit Bhole /Suryoun Lee
...
export default class AddNewProfile extends React.Component {
  ...
  async deleteProfile() {
    ...
    profiles.filter((item) => item.name !== name);
  ...
  async save() {
    ...
    profiles.map((c) => {
      if (c.name == this.state.name) {
        Alert.alert("This name is already existed. Please try other name.");
        check = 2;
      }
    });
    if (check == 1) {
        ...
        <FloatingAction
          ...
          onPressItem={name => {
            if (name == "back") {
              ...
              //added
              this.deleteProfile(this.state.name);
